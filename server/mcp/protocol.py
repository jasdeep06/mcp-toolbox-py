...
class McpServer:
    ...
    def __init__(self, tools: Dict[str, Any], toolsets: Dict[str, Toolset]):
        ...
        if "" not in self.toolsets:
            self.toolsets[""] = Toolset(
                "", self.tools, "Default toolset with all tools"
            )

    async def handle_request(
        self, request_data: str, toolset_name: str = ""
    ) -> Optional[str]:
    ...
    async def _process_request(
        self, request: McpRequest, toolset_name: str
    ...
    async def _handle_list_tools(
        ...
        return McpResponse(jsonrpc="2.0", id=request.id, result={"tools": tools_list})
    ...
    async def _handle_call_tool(
        ...
        if not toolset.has_tool(tool_name):
            return McpResponse(
                ...
                error={
                    "code": -32602,
                    "message": f"Tool not found in toolset: {tool_name}",
                },
            )
        ...
        try:
            ...
            return McpResponse(
                ...
                result={
                    "content": [
                        {"type": "text", "text": json.dumps(results, default=str)}
                    ]
                },
            )
        except Exception as e:
