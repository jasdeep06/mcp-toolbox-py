...
class PostgresSqlConfig(ToolConfig):
    ...
    def create_tool(self, sources: Dict[str, Source]) -> 'PostgresSqlTool':
        if self.source not in sources:
            raise ValueError(f"Source '{self.source}' not found")
        
        source = sources[self.source]
        if not isinstance(source, PostgresSource):
            raise ValueError(f"Source '{self.source}' must be a PostgreSQL source")
        
        return PostgresSqlTool(
            name=self.name,
...
class PostgresSqlTool(Tool):
    """Tool for executing parameterized SQL statements."""
    
    def __init__(self, name: str, description: str, statement: str,
                 parameter_set: ParameterSet, source: PostgresSource,
                 auth_required: List[str] = None):
        super().__init__(name, "postgres-sql", description, parameter_set, auth_required)
...
def create_postgres_sql_config(name: str, config_data: Dict) -> PostgresSqlConfig:
    """Factory function for PostgreSQL SQL tool."""
